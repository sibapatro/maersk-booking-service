...
spring:
  ...
  data:
    mongodb:
      uri: mongodb://localhost:27017/maersk_booking
  webflux:
    multipart:
...
logging:
  level:
    com.maersk.booking: DEBUG
    org.springframework.web.reactive.function.client: DEBUG

external:
  maersk:
    api:
      base-url: https://maersk.com/api
      check-available-endpoint: /bookings/checkAvailable
      timeout: 5000
      max-retries: 3
      backoff-period: 1000
...
COPY target/maersk-container-booking-*.jar app.jar
...
services:
  ...
  app:
    build: .
    container_name: maersk-booking-service
    ports:
      - "8080:8080"
    depends_on:
      - mongo
    environment:
      - SPRING_DATA_MONGODB_URI=mongodb://mongo:27017/maersk_booking
    healthcheck:
